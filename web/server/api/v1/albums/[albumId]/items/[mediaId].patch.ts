/**
 * PATCH /api/v1/albums/:albumId/items/:mediaId
 *
 * Update an album item's caption.
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<{ caption?: string | null }>(event)
  if (!('caption' in body)) throw createError({ statusCode: 400, message: 'caption is required' })
